# Konsider Phase 3A — Criteria Research Framework and Prioritisation

**Status:** Phase 3A framework complete
**Version:** 1.0
**Date:** 24 July 2026
...
Phase 3 should not treat the 84-item search space as 84 equally expensive investigations.

The approved research strategy is:

1. **Classify and order all 84 candidates now.**
2. **Lightly screen all 84 in Phase 3B.**
3. **Deeply research candidates in balanced batches of 12–15 in Phase 3C.**
...
5. **Keep the current production baseline unchanged during Phase 3A:** an enabled ranking criterion should, by default, be complete and ready for all 91 currently supported countries.
...
The desired Phase 3 outcome remains approximately **10–15 total enabled criteria**, including the existing five, plus a small reserve set.
...
Phase 3A does **not** change these principles.
...
| Pillar | User question | Existing coverage | Phase 3 priority |
| --- | --- | --- | --- |
...
| Band | Validated countries | Phase 3 interpretation |
| --- | ---: | --- |
...
A criterion must not pass by recomputing a smaller country universe unless Phase 3F explicitly approves that product change.
...
These are research defaults, not final production rules. Phase 3F must approve any criterion-specific freshness rule.
...
- overlap with other Phase 3 candidates;
...
After Phase 3B, calculate a **research priority score** only for ordering work:
...
Output: approximately 35–45 candidates across all Phase 3C batches.
...
| 74 | C24 | Overall affordability | Tax, cost and financial conditions | IC, DO | NATIONAL | EXISTING_MONITOR | Already represented by Konsider's broad household price-level criterion; retain as baseline, not new Phase 3 research. |
...
| 78 | C63 | Infrastructure readiness composite | Infrastructure and daily-life reliability | IC, DO | NATIONAL | EXISTING_MONITOR | Existing experimental composite; Phase 3 should evaluate components and redundancy rather than treat it as a new criterion. |
...
- enough repeated patterns to justify the minimal Phase 3D probe framework.
...
The second batch should be revised after Phase 3B and the first Phase 3C findings.
...
| Status | Meaning |
| --- | --- |
| `NOT_SCREENED` | No Phase 3 assessment yet. |
...
| `DEEP_RESEARCH_APPROVED` | Included in a Phase 3C batch. |
...
## 12. Candidate source families for Phase 3B
...
Phase 3B must replace these families with exact dataset candidates.

---

## 13. Standard Markdown report schema

Every Phase 3C criterion report should use this structure:
...
### Phase 3D must not begin as a generic platform project

It should begin only after Phase 3C reveals repeated technical patterns. The first implementation should support two or three representative source types and reuse the repository's existing raw artifact, attempt, mapping and report conventions.

### Phase 3F owns guardrail changes

Phase 3A deliberately leaves these questions open:
...
No later implementation should assume an answer before Phase 3F.

---

## 16. Phase 3A completion criteria
