...
SOURCE_RULES = {
    "ambient_pm25_population_weighted": (
        {"modelled"},
        {"wdi_distribution", "modelled_estimate", "cross_country_comparison_only"},
    ),
    "intentional_homicide_rate": (
        {"reported_or_estimated"},
        {"wdi_distribution", "secondary_distribution", "cross_country_comparability_caution"},
    ),
    ...
    "women_legal_economic_equality": (
        {"composite"},
        {"world_bank_primary_dataset", "de_jure_legal_framework", "not_de_facto_outcomes"},
    ),
    "infrastructure_readiness_composite": (
        {"derived_composite"},
        {"wdi_distribution", "equal_weight_three_components", "mixed_reference_years"},
    ),
    ...
    *,
    ...
    if schema_version != RELEASE_SCHEMA_VERSION:
        issue(
            "incompatible_schema",
            "error",
            f"Expected {RELEASE_SCHEMA_VERSION}, got {schema_version}.",
        )

    for source in sources:
        if source.license_name != "Creative Commons Attribution 4.0 International":
            issue(
                "licence_not_product_ready",
                "blocker",
                f"{source.source_id} lacks audited CC BY 4.0.",
                scope="product_readiness",
                metric=source.criterion_id,
            )
        if (
            "may be redistributed" not in source.redistribution
            and "explicitly licensed" not in source.redistribution
        ):
            issue(
        ...
        if not source.license_url or not source.license_evidence or not source.attribution:
            issue(
                "licence_contract_incomplete",
                "error",
                f"Licence evidence incomplete for {source.source_id}.",
                metric=source.criterion_id,
            )
    ...
    for artifact in artifacts:
        ...
        if artifact.http_status < 200 or artifact.http_status >= 300 or not artifact.final_url:
            issue(
                "invalid_http_metadata",
                "error",
                f"HTTP retrieval metadata invalid for {artifact.artifact_id}.",
            )
        try:
        ...
        if source and artifact.parser_version != source.parser_version:
            issue(
                "artifact_parser_version_mismatch",
                "error",
    ...
    for observation in observations:
        ...
        if country not in COUNTRIES:
            issue(
                "unknown_country",
                "error",
                "Country outside experiment set.",
                country=country,
                metric=metric,
            )
        if keys[(country, metric, observation.reference_end)] > 1:
            issue(
                ...
                "error",
                "Duplicate country/metric/period.",
                country=country,
                metric=metric,
            )
        if sources and (not source or source.criterion_id != metric):
            issue(
                ...
                "error",
                "Observation source is not registered for criterion.",
                country=country,
                metric=metric,
            )
        elif source and observation.parser_version != source.parser_version:
            issue(
                "observation_parser_version_mismatch",
                "error",
        ...
        if len(observation.raw_artifact_ids) != len(set(observation.raw_artifact_ids)):
            issue(
                ...
                "error",
                "Duplicate raw artifact reference.",
                country=country,
                metric=metric,
            )
        record_artifacts = {record.artifact_id for record in observation.source_records}
        if (
            not observation.source_records
            or set(observation.raw_artifact_ids) != record_artifacts
            or not record_artifacts <= artifact_by_id.keys()
        ):
            issue(
                "incomplete_record_provenance",
                "error",
                "Exact artifact and source record lineage is required.",
                country=country,
                metric=metric,
            )
        if (
            any(not record.locator or not record.record_id for record in observation.source_records)
            or not observation.method_version
        ):
            issue(
                "incomplete_method_provenance",
                "error",
                "Record ID/locator and method versions are required.",
                country=country,
                metric=metric,
            )
        if REQUIRED_UNITS.get(metric) != observation.unit:
            issue(
                "invalid_unit",
                "error",
                f"Expected {REQUIRED_UNITS.get(metric)}, got {observation.unit}.",
                country=country,
                metric=metric,
            )
        ...
        if not low <= observation.value <= high:
            issue(
                "implausible_value",
                "error",
                f"Value {observation.value} outside [{low}, {high}].",
                country=country,
                metric=metric,
            )
        ...
        if observation.components:
            component_artifacts = {
                component.source_record.artifact_id for component in observation.components
            }
            if not component_artifacts <= record_artifacts or any(
                component.reference_year <= 0 for component in observation.components
            ):
                issue(
                    "component_provenance_invalid",
                    "error",
                    "Derived components must reference exact source records and years.",
                    country=country,
                    metric=metric,
                )
        if (
            metric == "household_consumption_price_level_us_100"
            and len(observation.components) != 2
        ):
            issue(
                "icp_component_contract",
                "error",
                "ICP PLI requires PPP and exchange-rate components.",
                country=country,
                metric=metric,
            )
        if metric == "infrastructure_readiness_composite":
            ...
            if component_ids != expected:
                issue(
                    "infrastructure_component_contract",
                    "error",
    ...
    expected_attempts = {
        (source.source_id, source.criterion_id, code) for source in sources for code in COUNTRIES
    }
    ...
    if sources and expected_attempts != actual_attempts:
        issue(
            ...
            "error",
            f"Attempt matrix missing {len(expected_attempts - actual_attempts)}, extra {len(actual_attempts - expected_attempts)}.",
        )
    ...
    for attempt in attempts:
        if attempt.status not in {"success", "no_data", "failed", "rejected"}:
            issue(
                ...
                metric=attempt.criterion_id,
            )
        if attempt.status == "success" and (
            not attempt.observation_id or attempt.observation_id not in observation_ids
        ):
            issue(
                "attempt_observation_missing",
                "error",
                "Successful attempt must reference an observation.",
                country=attempt.country_code,
                metric=attempt.criterion_id,
            )
        if attempt.status != "success" and not attempt.reason:
            issue(
                "attempt_reason_missing",
                "error",
                "Non-success attempt needs a reason.",
                country=attempt.country_code,
                metric=attempt.criterion_id,
            )
    ...
    if len(coverage) < min_criteria:
        issue(
            "insufficient_criteria", "error", f"Only {len(coverage)} criteria; need {min_criteria}."
        )
    ...
    for score in scores:
        if not 1 <= score.score <= 10 or not set(score.input_observation_ids) <= observation_ids:
            issue(
                "invalid_score_lineage",
                "error",
                "Score range or lineage invalid.",
                country=score.country_code,
                metric=score.criterion_id,
            )
        if score_keys[(score.country_code, score.criterion_id)] > 1:
            issue(
                "duplicate_score",
                "error",
                "Duplicate country/criterion score.",
                country=score.country_code,
                metric=score.criterion_id,
            )
        ...
        if expected_method and score.method_version != expected_method:
            issue(
                "scoring_method_not_product_ready",
                "blocker",
                f"Expected {expected_method}, got {score.method_version}.",
                scope="product_readiness",
                country=score.country_code,
                metric=score.criterion_id,
            )
    ...
    if sources:
        for metric, source in source_by_metric.items():
            ...
            if max_age is not None:
                for row in rows:
                    years = [component.reference_year for component in row.components] or [
                        int(row.reference_end[:4])
                    ]
                    if max(as_of_year - year for year in years) > max_age:
                        issue(
                            "source_stale",
                            "blocker",
                            f"Source year(s) {sorted(years)} exceed {max_age}-year freshness limit.",
                            scope="product_readiness",
            ...
            if old and old.value != 0:
                ...
                if change > 0.25:
                    issue(
                        ...
                        "warning",
                        f"Value changed {change:.1%} from previous release.",
                        scope="product_readiness",
    ...
    structural_passed = not any(
        item.severity == "error" and item.scope == "structural" for item in issues
    )
    criterion_readiness = {}
    for metric in sorted(source_by_metric):
        success_count = sum(
            item.criterion_id == metric and item.status == "success" for item in attempts
        )
        blockers = [
            item
            for item in issues
            if item.metric_id == metric and item.severity in {"error", "blocker"}
        ]
        criterion_readiness[metric] = (
            structural_passed
            and len(coverage.get(metric, set())) >= product_country_coverage
            and success_count >= product_country_coverage
            and not blockers
        )
    ...
    if sources and ready_count < product_min_ready:
        issue(
            "insufficient_product_ready_criteria",
            "blocker",
            f"Only {ready_count} criteria ready; need {product_min_ready}.",
            scope="product_readiness",
        )
    return ValidationReport(
        ...
        status_counts=dict(Counter(item.status for item in attempts)),
        issues=tuple(issues),
    )
