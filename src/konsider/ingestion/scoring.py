...
CURRENT_THRESHOLD_METHODS = {
    **LEGACY_THRESHOLD_METHODS,
    "household_consumption_price_level_us_100": ("icp_relative_cost_bands_v2", ((50, 9), (75, 8), (100, 6), (125, 4), (150, 2), (175, 1))),
    "women_legal_economic_equality": ("wbl_legal_equality_bands_v1", ((40, 1), (55, 3), (70, 5.5), (85, 8), (100, 10))),
    "infrastructure_readiness_composite": ("infrastructure_readiness_bands_v1", ((30, 1), (45, 3), (60, 5.5), (75, 8), (90, 10))),
}
...
def _algorithm_scores(
    values: list[float], algorithm: str, direction: str, anchors=None,
    *, metric_id: str | None = None, broad_icp: bool = False,
) -> list[float]:
...
def score_observations(
    observations: list[MetricObservation], *, profile: str = "current",
) -> list[MetricScore]:
    ...
    for metric_id, rows in sorted(grouped.items()):
        ...
        transformed = _algorithm_scores(
            [row.value for row in rows], "threshold", direction, anchors,
            metric_id=metric_id, broad_icp=broad_icp,
        )
        transform = "fixed_broad_relative_cost_bands" if broad_icp and metric_id == "household_consumption_price_level_us_100" else "fixed_piecewise_linear_thresholds"
...
def _infrastructure_experiment(rows: list[MetricObservation]) -> dict[str, object]:
    ...
    series = {
        component_id: [next(c.value for c in row.components if c.component_id == component_id) for row in rows]
    ...
    component_years = {
        component_id: sorted({next(c.reference_year for c in row.components if c.component_id == component_id) for row in rows})
        for component_id in component_ids
    }
    return {
        "components": component_ids, "component_reference_years": component_years,
...
def sensitivity_experiments(observations: list[MetricObservation]) -> dict[str, object]:
    ...
    for metric_id, unsorted_rows in sorted(grouped.items()):
        ...
        algorithms = {
            name: _algorithm_scores(values, name, direction, anchors, metric_id=metric_id, broad_icp=broad_icp)
        ...
        trimmed_threshold = _algorithm_scores(trimmed, "threshold", direction, anchors, metric_id=metric_id, broad_icp=broad_icp)
        ...
        cluster_spreads = {
            ...
            for name in ("winsorized_minmax", "percentile_rank", "threshold")
            for scores in [_algorithm_scores(clustered, name, direction, anchors, metric_id=metric_id, broad_icp=broad_icp)]
        }
        result = {
            "selected_method_version": method_version, "anchors": [list(item) for item in anchors],
        ...
        if broad_icp:
            result["selection_reason"] = "Discrete broad bands prevent ICP extrapolations from implying a precise country ranking."
