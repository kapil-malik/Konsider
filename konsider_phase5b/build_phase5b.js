...
const masterPath = path.join(root, "konsider_phase5a", "phase5a_master_research_list.json");
...
C63	Combined digital and logistics readiness.	Existing Konsider inputs; World Bank/ITU	Existing experimental infrastructure readiness composite		C2	YES	FULL	Existing release cadence	LOW	unsuitable independent	DEFERRED	NO FURTHER WORK	Already available experimentally; Phase 5 should research its components rather than duplicate the composite.	RED_EXISTING_CRITERION|RED_SHARED_COMPONENTS
...
const rows = raw.split(/\r?\n/).map((line) => {
  ...
  return {
    ...
    phase5a_granularity: source.natural_granularity,
    natural_granularity,
...
const output = {
  ...
  fact_estimate_legend: {
    ...
    PRELIMINARY_ESTIMATE: "Fit, comparability, stable-universe coverage, licensing fit, and priority remain hypotheses for Phase 5C unless explicitly stated otherwise."
  },
...
fs.writeFileSync(path.join(__dirname, "phase5b_screening.json"), `${JSON.stringify(output, null, 2)}\n`, "utf8");

const csvHeaders = [
  "criterion_id","source_item_number","name","category","measures","classification_tags",
  "phase5a_granularity","natural_granularity","likely_authoritative_publisher_families",
...
fs.writeFileSync(path.join(__dirname, "phase5b_screening.csv"), `${csv}\r\n`, "utf8");
...
| Recurring family | Best-fit criterion groups | Verified family fact | Main Phase 5C questions |
| --- | --- | --- | --- |
...
For each selected criterion, Phase 5C should replace family names with exact dataset/table/series candidates; capture dataset-specific terms; measure fresh/validated coverage against all 91 ISO codes; record missingness by region; and preserve legal, survey, model and city-layer limitations separately. No criterion in this report is production-approved.
`;
fs.writeFileSync(
  path.join(__dirname, "phase5b_screening.md"),
  `${report.trimEnd()}\n`,
  "utf8",
);
