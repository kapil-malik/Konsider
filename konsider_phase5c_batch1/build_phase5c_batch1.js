...
const specs = [
  ...
  },
  {
    ...
    evidence: [
      ...
      ev("No current official global dataset was verified that converts individual skilled-work eligibility into comparable destination scores.", "Phase 5C source search", "ESTIMATED")
    ]
  },
  {
    ...
    evidence: [
      ...
      ev("No authoritative, current, global route-level permanent-residence dataset was verified.", "Phase 5C source search", "ESTIMATED")
    ]
  },
  {
...
const output = {
  phase_id: "5C",
  batch_id: "PHASE5C_BATCH1_2026-07-24",
...
const csvRows = specs.map(s => {
  const p = s.sources[0];
  const track = s.shortlist || "NO_FURTHER_PHASE5E_WORK";
...
const table = specs.map(s => {
  ...
  return `| ${s.rank} | ${s.id} | ${s.status} | ${s.granularity} | ${c} | ${s.sources[0].authority_grade}/${s.sources[0].licence_grade}/${s.sources[0].comparability_grade}/${s.scoring.grade} | ${s.shortlist || "No further 5E work"} |`;
}).join("\n");

const md = `# Konsider Phase 5C — Batch 1 source-feasibility research
...
fs.writeFileSync(path.join(OUT, "phase5c_batch1.json"), JSON.stringify(output, null, 2) + "\n");
fs.writeFileSync(path.join(OUT, "phase5c_batch1.csv"), csvHeaders.map(esc).join(",") + "\n" + csvRows.join("\n") + "\n");
fs.writeFileSync(path.join(OUT, "phase5c_batch1.md"), `${md.trimEnd()}\n`);

// Lightweight local validation against the Phase 5A record schema. This is report QA,
// not production ingestion code.
const schema = JSON.parse(fs.readFileSync(path.join(OUT, "..", "konsider_phase5a", "phase5a_research_record.schema.json"), "utf8"));
...
if (validationErrors.length) {
  ...
  console.log(`Wrote and schema-validated ${records.length} records; ${shortlist.length} Phase 5E shortlist entries.`);
}
